...
JsonData UserAuthorizerHandler::handle(const JsonData& jsonData) {
    ...
    try {
        ...
        UsersData userData = userTable.SELECT(user.login);
       ...
       if (user.login != userData.login) {
            ...
            jsonDataNew.users.back().name = userData.name;
            jsonDataNew.users.back().surname = userData.surname;
            jsonDataNew.users.back().userId = userData.userId;

            userTable.UPDATE(userData.userId, "is_authorized", "1");
            ...
            if (!messages.empty()) {
                userTable.UPDATE(jsonDataNew.messages[jsonDataNew.messages.size()  - 1].date, userData.userId);
            }
        }
    }
...
JsonData StartChatHandler::handle(const JsonData &jsonData) {
    ...
    try {
        ...
        std::vector<UsersData> usersData = userTable.SELECT("login", jsonData.users[0].login);
        ...
        if (!usersData.empty()) {
            ...
            jsonDataNew.users[0].userId = usersData[0].userId;
        }
    }
    catch (MySQLDBExceptions& e) {
        std::cout << e.what() << std::endl;
